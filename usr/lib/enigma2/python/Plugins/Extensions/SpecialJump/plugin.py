...
from enigma import eTimer, ePoint, eSize, eDVBVolumecontrol, iServiceInformation, iPlayableService, iRecordableService, eDBoxLCD, pNavigation, eServiceReference
...
config.plugins.SpecialJump.disablePreloadIPTV = ConfigYesNo(default=False)
...
def autostart(reason, **kwargs):
	...
	if config.plugins.SpecialJump.enable.value:
		print("SpecialJump enabled: %s" % config.plugins.SpecialJump.enable.getValue())
...
def InfoBarPlugins__init__(self):
	if isinstance(self, InfoBarShowMovies):
		if config.plugins.SpecialJump.debugEnable.getValue():
			print("isinstance(self, InfoBarShowMovies)")
		...
		if config.plugins.SpecialJump.debugEnable.getValue():
			print("isinstance(self, InfoBarEPG)")
...
def specialjump_forwards(self, mode, size):
	if size == 'small':
		SpecialJump.specialJumpForwards(SpecialJumpInstance, self, mode, size, int(config.plugins.SpecialJump.smallSpecialJumpStart.getValue()))
	else:
		SpecialJump.specialJumpForwards(SpecialJumpInstance, self, mode, size, 0)


def specialjump_backwards(self, mode, size):
	if size == 'small':
		SpecialJump.specialJumpBackwards(SpecialJumpInstance, self, mode, size, int(config.plugins.SpecialJump.smallSpecialJumpStart.getValue()))
	else:
		SpecialJump.specialJumpBackwards(SpecialJumpInstance, self, mode, size, 0)


def specialjump_jump(self, mode, jumpkey):
	#SpecialJump.showTimeBetweenKeys(SpecialJumpInstance)
	if jumpkey == 'J1':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "-", int(config.plugins.SpecialJump.jump1.getValue()), config.plugins.SpecialJump.jump1action.getValue())
	elif jumpkey == 'J2':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "-", int(config.plugins.SpecialJump.jump2.getValue()), config.plugins.SpecialJump.jump2action.getValue())
	elif jumpkey == 'J3':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "-", int(config.plugins.SpecialJump.jump3.getValue()), config.plugins.SpecialJump.jump3action.getValue())
	elif jumpkey == 'J4':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "-", int(config.plugins.SpecialJump.jump4.getValue()), config.plugins.SpecialJump.jump4action.getValue())
	elif jumpkey == 'J5':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "-", int(config.plugins.SpecialJump.jump5.getValue()), config.plugins.SpecialJump.jump5action.getValue())
	elif jumpkey == 'J6':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "-", int(config.plugins.SpecialJump.jump6.getValue()), config.plugins.SpecialJump.jump6action.getValue())
	elif jumpkey == 'J7':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "-", int(config.plugins.SpecialJump.jump7.getValue()), config.plugins.SpecialJump.jump7action.getValue())
	elif jumpkey == 'J8':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "-", int(config.plugins.SpecialJump.jump8.getValue()), config.plugins.SpecialJump.jump8action.getValue())
	elif jumpkey == 'K1':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "1", int(-config.seek.selfdefined_13.getValue()), "nothing")
	elif jumpkey == 'K4':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "4", int(-config.seek.selfdefined_46.getValue()), "nothing")
	elif jumpkey == 'K7':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "7", int(-config.seek.selfdefined_79.getValue()), "nothing")
	elif jumpkey == 'K3':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "3", int(config.seek.selfdefined_13.getValue()), "nothing")
	elif jumpkey == 'K6':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "6", int(config.seek.selfdefined_46.getValue()), "nothing")
	elif jumpkey == 'K9':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "9", int(config.seek.selfdefined_79.getValue()), "nothing")
	elif jumpkey == 'K2':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "2", int(0), "nothing")
	elif jumpkey == 'K5':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "5", int(0), "nothing")
	elif jumpkey == 'K8':
		SpecialJump.fixedJump(SpecialJumpInstance, self, mode, "8", int(0), "nothing")
...
class SpecialJumpConfiguration(Screen, ConfigListScreen):
	...
	def initConfigList(self):
		...
		(_("Disable preload for IPTV"), 5, config.plugins.SpecialJump.disablePreloadIPTV),
		(_("Enable zap speed measurement"), 6, config.plugins.SpecialJump.zapspeed_enable),
		(_("Zap speed infobox verbosity"), 7, config.plugins.SpecialJump.zapspeedVerbosity),
		(_("Zap speed infobox timeout"), 8, config.plugins.SpecialJump.zapspeedTimeout_ms),
		(_("Zap speed infobox x position"), 9, config.plugins.SpecialJump.zapspeed_x),
		(_("Zap speed infobox y position"), 10, config.plugins.SpecialJump.zapspeed_y),
		(_("Zap speed infobox anchor"), 11, config.plugins.SpecialJump.zapspeed_anchor),
		(_("Zap speed measurement timeout (zap error) [ms]"), 12, config.plugins.SpecialJump.zapspeedMeasureTimeout_ms),
		(_("Auto-zap benchmark mode time between zaps [ms]"), 13, config.plugins.SpecialJump.fastZapBenchmarkTime_ms),
...
class SpecialJump():
	...
	def __init__(self, session):
		...
		self.services_hd_plus = ['1:0:19:151A:455:1:C00000:0:0:0:', '1:0:19:2E9B:411:1:C00000:0:0:0:', '1:0:19:2EAF:411:1:C00000:0:0:0:', '1:0:19:5274:41D:1:C00000:0:0:0:', '1:0:19:EF10:421:1:C00000:0:0:0:', '1:0:19:EF11:421:1:C00000:0:0:0:', '1:0:19:EF14:421:1:C00000:0:0:0:', '1:0:19:EF15:421:1:C00000:0:0:0:', '1:0:19:EF74:3F9:1:C00000:0:0:0:', '1:0:19:EF75:3F9:1:C00000:0:0:0:', '1:0:19:EF76:3F9:1:C00000:0:0:0:', '1:0:19:EF77:3F9:1:C00000:0:0:0:', '1:0:19:EF78:3F9:1:C00000:0:0:0:', '1:0:19:1519:455:1:C00000:0:0:0:', '1:0:19:157C:41F:1:C00000:0:0:0:', '1:0:19:1581:41F:1:C00000:0:0:0:', '1:0:19:2777:409:1:C00000:0:0:0:', '1:0:19:30D6:413:1:C00000:0:0:0:']
		self.services_hd_free = ['1:0:19:283D:3FB:1:C00000:0:0:0:', '1:0:19:283E:3FB:1:C00000:0:0:0:', '1:0:19:283F:3FB:1:C00000:0:0:0:', '1:0:19:2859:401:1:C00000:0:0:0:', '1:0:19:285B:401:1:C00000:0:0:0:', '1:0:19:286E:425:1:C00000:0:0:0:', '1:0:19:286F:425:1:C00000:0:0:0:', '1:0:19:2870:425:1:C00000:0:0:0:', '1:0:19:2873:425:1:C00000:0:0:0:', '1:0:19:2887:40F:1:C00000:0:0:0:', '1:0:19:2888:40F:1:C00000:0:0:0:', '1:0:19:2889:40F:1:C00000:0:0:0:', '1:0:19:2B66:3F3:1:C00000:0:0:0:', '1:0:19:2B7A:3F3:1:C00000:0:0:0:', '1:0:19:2B84:3F3:1:C00000:0:0:0:', '1:0:19:2B8E:3F2:1:C00000:0:0:0:', '1:0:19:2B98:3F2:1:C00000:0:0:0:', '1:0:19:2BA2:3F2:1:C00000:0:0:0:', '1:0:19:6EA5:4B1:1:C00000:0:0:0:', '1:0:1:1389:3EA:1:C00000:0:0:0:']
	...
	def powerOn(self):
		...
		if config.plugins.SpecialJump.EMCdirsHideOnPowerup.getValue():
			try:
				...
				if config.plugins.SpecialJump.debugEnable.getValue():
					print("could not set config.EMC.cfghide_enable True")
	...
	def gotRecordEvent(self, service, event):
		if event == iRecordableService.evRecordStopped:
			if config.plugins.SpecialJump.debugEnable.getValue():
				print("evRecordStopped starting timer")
	...
	def gotRecordEventDelayed(self):
		...
		if len(NavigationInstance.instance.getRecordings(False, pNavigation.isFromSpecialJumpFastZap)) == 0:
			...
			if config.plugins.SpecialJump.debugEnable.getValue():
				print("evRecordStopped externally, nulling the pointer of the pseudo recording")
	...
	def specialJumpStartTimerShowZapspeedBox(self, ind1):
		...
		if config.plugins.SpecialJump.debugEnable.getValue():
			print("Zap Speed: %s zap t=%sms" % (self.zap_list_ind1[ind1], self.zap_time))
	...
	def specialJumpZapspeedPollTimeout(self):
		...
		if self.zap_time_event_counter == config.plugins.SpecialJump.zapspeedMeasureTimeout_ms.getValue() / self.zap_time_event_counter_ms:
			if self.zap_time_res_0_seen:
				...
				if config.plugins.SpecialJump.debugEnable.getValue():
					print("could not determine zap time (yres 0 not seen until timeout)")
				self.SJZapspeedPollTimer.stop()
		else:
	...
	def debugMessage(self, locationString):
		if config.plugins.SpecialJump.debugEnable.getValue():
			print('-----------------------------------------')
			print("[SpecialJump] %s" % locationString)
			print('-----------------------------------------')
			seek = self.getSeek()
			print('self.InfoBar_instance: %s' % str(self.InfoBar_instance))
			print('seek:', seek)
	...
	def activateTimeshiftIfNecessaryAndDoSeekRelative(self, pts, MuteTime_ms):
		if config.plugins.SpecialJump.debugEnable.getValue():
			print('activateTimeshiftIfNecessaryAndDoSeekRelative', pts, ' ', MuteTime_ms)
		if InfoBar and self.InfoBar_instance:
			seek = self.getSeek()
			if seek is not None:
			...
			if self.infoBarShownAndEnableSpecialJump2(True):  # True, first activation only if paused
				...
				if self.isSeekstatePaused():
					if config.plugins.SpecialJump.EnableSpecialJump2.value:
						if config.plugins.SpecialJump.debugEnable.getValue():
							print("channelDown calls showAfterSeek")
	...
	def channelUp(self, parent, mode):
		...
		if mode == "MP":
			if self.InfoBar_instance.seekstate[1] or self.InfoBar_instance.seekstate[2]:  # cue or slowmotion
				...
				self.unPauseService()
				if config.plugins.SpecialJump.debugEnable.getValue():
					print("channelUp calls showAfterSeek")
				InfoBarSeek.showAfterSeek(self.InfoBar_instance)
		if mode == "TV":
			if self.isCurrentlySeekable():  # timeshift active and play position "in the past"
				if self.InfoBar_instance.seekstate[1] or self.InfoBar_instance.seekstate[2]:  # cue or slowmotion
	...
	def zapHandler(self, direction):
		if config.plugins.SpecialJump.fastZapEnable.value and self.zapPredictiveService is not None:
			...
			if cur:
				...
				if cur == self.zapPredictiveService:
					...
					if config.plugins.SpecialJump.debugEnable.getValue():
						print(self.fastZapDirection, " PIP predictive zap success")
					self.zap_success = 'fast'
				else:
					self.zap_success = 'miss'
					if config.plugins.SpecialJump.debugEnable.getValue():
						print(self.fastZapDirection, " PIP predictive zap guessed wrong, is ", cur, " exp. ", self.zapPredictiveService)
		else:
			self.zap_success = 'off'
		...
		if config.plugins.SpecialJump.fastZapEnable.value and self.getNumberOfFrontendsFreeForSJ() > 1 and not (self.isIPTV() and config.plugins.SpecialJump.disablePreloadIPTV.value):
			...
			if (config.plugins.SpecialJump.fastZapMethod.value == "pip") or (config.plugins.SpecialJump.fastZapMethod.value == "pip_hidden"):
				if (self.fastZapPipActive == False):
					if (InfoBarPiP.pipShown(self.InfoBar_instance) == False):
						if config.plugins.SpecialJump.debugEnable.getValue():
							print(direction, " zapPredictive incl. initial showPiP")
						self.postZap_preloadPredictive()
					else:
						if config.plugins.SpecialJump.debugEnable.getValue():
							print(direction, " PIP unexpectedly active, don't touch PIP", InfoBarPiP.pipShown(self.InfoBar_instance))
				else:
					if (InfoBarPiP.pipShown(self.InfoBar_instance) == True):
						if config.plugins.SpecialJump.debugEnable.getValue():
							print(direction, " zapPredictive (PIP already active)")
						self.postZap_preloadPredictive()
					else:
						if config.plugins.SpecialJump.debugEnable.getValue():
							print(direction, " zapPredictive (PIP unexpectedly inactive)", InfoBarPiP.pipShown(self.InfoBar_instance))
						self.postZap_preloadPredictive()
			else:
				if config.plugins.SpecialJump.debugEnable.getValue():
					print(direction, " start zapPredictive (pseudo rec.)")
				self.postZap_preloadPredictive()
		else:
			if config.plugins.SpecialJump.debugEnable.getValue():
				print("disabling FastZap temporarily (only %d free tuner(s) available)" % self.getNumberOfFrontendsFreeForSJ())
	...
	def postZap_preloadPredictive(self):
		...
		if self.compareTSIDandNamespace(storeService, fastZapNextService) and not config.plugins.SpecialJump.preloadIfSameTSID.value:
			if config.plugins.SpecialJump.debugEnable.getValue():
				print("FastZap not preloading the next service as it is on the same transponder")
			self.zapPredictiveService = None
		else:
		...
		if config.plugins.SpecialJump.debugEnable.getValue():
			print("zapPredictive 2")

	def enablePredictiveRecOrPIP(self, fastZapNextService):
		if config.plugins.SpecialJump.debugEnable.getValue():
			print("enablePredictiveRecOrPIP ", fastZapNextService.toString())
		if (config.plugins.SpecialJump.fastZapMethod.value == "pip") or (config.plugins.SpecialJump.fastZapMethod.value == "pip_hidden"):
			if (InfoBarPiP.pipShown(self.InfoBar_instance) == False):
				...
				if fastZapNextService:
					...
					if config.plugins.SpecialJump.debugEnable.getValue():
						print("zapPredictive ", self.fastZapDirection, " switched PIP to service ", fastZapNextService.toString())
			...
			if self.fastZapRecService is not None:
				...
				if config.plugins.SpecialJump.debugEnable.getValue():
					print("zapPredictive ", self.fastZapDirection, " pseudo recording service ", fastZapNextService.toString())

	def disablePredictiveRecOrPIP(self):
		if config.plugins.SpecialJump.debugEnable.getValue():
			print("disablePredictiveRecOrPIP")
	...
	def compareTSIDandNamespace(self, service1, service2):
		...
		if (namespace1 and tsid1 and namespace2 and tsid2):
			if config.plugins.SpecialJump.debugEnable.getValue():
				print("check whether next service is on same transponder: (%04x,%04x) vs. (%04x,%04x)" % (namespace1, tsid1, namespace2, tsid2))
	...
	def handleOKlong(self, parent, mode):
		...
		if self.SJMode == "TV":
			...
			if config.plugins.SpecialJump.debugEnable.getValue():
				print("handleOKlong not implemented in MP mode")
	...
	def isIPTV(self):
		...
		if ref is not None:
			try:
				...
				mypath = ''
		if (mypath.startswith('http://')) or (mypath.startswith('https://')):
			return True
		else:
			return False
